(function (b, i, m, e) {
    b[m] = b[m] || {};
    var p = b[m], q = /(?:^|\/)([^(\/]+)(?=\(|$)/, c = /\(([^)]+)\)/, n = /loaded|complete/i,
        f = i.getElementsByTagName("script"), a = i.getElementsByTagName("base"), d = {}, k = [], j = [];
    var l = {
    ...
    var o = p.boot = {
        require: function (s, t, r) {
            ...
            var w = {name: v, dependList: t, waitList: [], callback: y};
            ...
            var r = arguments, s = this, t = "", x = l.map(w.dependList, function (y) {
    ...
    boot.declare("fe", "", function (Frame) {
        ...
        (function () {
            var idSeed = 0, ua = navigator.userAgent.toLowerCase(), check = function (r) {
                    return r.test(ua);
                }, isStrict = document.compatMode == "CSS1Compat", isOpera = check(/opera/), isChrome = check(/chrome/),
                isWebKit = check(/webkit/), isSafari = !isChrome && check(/safari/),
                isSafari3 = isSafari && check(/version\/3/), isSafari4 = isSafari && check(/version\/4/),
                isIE = !isOpera && check(/msie/), isIE6 = isIE && check(/msie 6/), isIE7 = isIE && check(/msie 7/),
                isIE8 = isIE && check(/msie 8/), isIE9 = isIE && check(/msie 9/), isGecko = !isWebKit && check(/gecko/),
                isGecko3 = isGecko && check(/rv:1\.9/), isBorderBox = isIE && !isStrict,
                isWindows = check(/windows|win32/), isMac = check(/macintosh|mac os x/), isAir = check(/adobeair/),
                isLinux = check(/linux/), isSecure = /^https/i.test(global.location.protocol);
            if (isIE6) {
                try {
        ...
        (function () {
            Frame.apply(Frame, {
                isTopReady: false, topReadyList: [], topReady: function (fn) {
                    if (Frame.isTopReady) {
                        fn.call(document, Frame);
                    } else {
...
(function (b, a, c) {
    boot.declare("modules", "config, extension.array, util.js", function (f, e, d, h, g) {
        f.modules = {
            init: function (m) {
                var j = "//j2.58cdn.com.cn/js/v6/source/", l = this.getConfigPath(m), k = m.configs[l],
...
(function (b, a, c) {
    ...
    boot.declare("config", "", function (p, v) {
        ...
        var h = k.modules, g = h == "home" || h == "homepage", s = h == "toplist", f = h == "list" || h == "listpage",
            d = h == "final" || h == "finalpage", r = h == "my" || h == "mypage", n = h == "post" || h == "postpage";
        var t = b.location.search.indexOf("debug=58") != -1,
            u = {ERROR: true, WARING: true, SUCCESS: b.location.search.indexOf("logsuccess=58") != -1};
        p.config = {
            j: k,
            city: i,
            cate: q,
        ...
        d.js = {
            cache: [], push: function (h, f, e, g) {
                ...
                if (arguments.length == 1 && c.isObject(arguments[0])) {
                    ...
                    this.appendScript({src: h, onsuccess: f, onerror: e, refresh: g});
                }
            }, jsonp: function (g, i, f, e, h) {
                if (arguments.length == 1 && c.isObject(arguments[0])) {
                    this.appendScript(arguments[0]);
                } else {
                    this.appendScript({src: g, params: i, type: "jsonp", onsuccess: f, onerror: e, refresh: h});
                }
            }, appendScript: function (i) {
                var o, r = this, i = i || {}, f = i.src, j = i.params || {}, s = i.onsuccess, n = i.onerror,
                    l = i.refresh, h = false, k = a.head || a.getElementsByTagName("head")[0],
                    p = /loaded|complete|undefined/i, e = a.dispatchEvent ? "onload" : "onreadystatechange";
                if (c.isEmpty(f)) {
                    return;
                }
                if (i.type == "jsonp") {
                    var g = j.callbackName || "callback", q = j.callback || c.id("jsonp");
                    f.indexOf("?") == -1 ? f += "?" : f += "&";
                    f += g + "=" + q;
                ...
                var h = window, g = h.document, i = g.dispatchEvent ? "onload" : "onreadystatechange", f = e.parentNode;
                if (f && f.nodeType === 1) {
